...
window.AI_STATE = {
  ...
  tasks: [
    ...
    },
    {
      ...
      log: [
        ...
        { time: "今日", text: "追加依頼：もう少し文字を大きくしたい→フォントサイズをT11と同じ1.6emに統一" },
        { time: "今日", text: "追加依頼：***文章***でオレンジ・サイズは変えない装飾を追加→3段階の装飾に対応完了" },
        { time: "今日", text: "追加依頼：add oneメルマガと同じ「お申し込みはこちら」ボタンも載せたい→[[文字|URL]]記法を移植。K2本文に直接書けばOK" },
        { time: "今日", text: "追加依頼：****文章****でオレンジ＋大きめを両方使いたい→4段階目の装飾を追加" },
        { time: "今日", text: "修正：***の役割をオレンジのみから、黒字のまま*より少し小さい中くらいのサイズ（1.3em）に変更" },
        { time: "今日", text: "追加依頼：Q2にヘッダー画像を入れたい→T11同様ドライブリンクを読み取って本文上部に表示する仕組みを追加" },
        { time: "今日", text: "質問：M2のチェックを外して送信すると全員に一斉送信されるか→はいと回答。T11同様の本送信前確認ダイアログを追加" },
        { time: "今日", text: "追加依頼：担当者名が空欄のとき宛名が「様」だけになってしまう→空欄なら自動で「ご担当者様」になるよう修正" },
        { time: "今日", text: "不具合報告：本送信でエラー停止。原因はJ列に「メール1（ブランドA）／メール2（ブランドB）」形式で複数メールが1セルに入っていたこと。有効なメールを自動抽出して個別送信＋失敗時もスキップして継続するよう修正" }
      ],
      deliverables: [
    ...
    },
    {
      ...
      status: "review",
      progress: 100,
      hint: "①再アプローチリストの一番左に新しい列を1本挿入（送信日用の空列）②GASスクリプトを貼り替え③テスト送信で見え方確認④D2のテストモードを外して本送信",
      cmd: "T11のスクリプトを見せて",
      log: [
        ...
        { time: "今日", text: "社長報告：今度は「ご担当者様」側に様が二重に。原因はA2本文側に手入力の様/ご担当者様が残っていた可能性が高いため、連続した様を自動で1個に畳み込む保険（collapseDuplicateHonorific_）を追加。A2の中身も要確認とご案内" },
        { time: "今日", text: "追加依頼：本文の一部だけフォントの大きさ・色を変えたい→**文章**とアスタリスクで囲むとオレンジ太字強調になる簡易記法を追加" },
        { time: "今日", text: "追加依頼：色は黒のままでサイズだけ大きくしたい箇所がある→*文章*（アスタリスク1つ）で色そのまま・太字・少し大きめにできる記法を追加" },
        { time: "今日", text: "追加依頼：画像と同じくらいまで文字を大きくしたい→強調文字のサイズを1.15em→1.6emに拡大" },
        { time: "今日", text: "修正：**は色だけ変えてサイズは本文と同じに戻し、*は色そのまま・1.6emで大きくのままに使い分けを整理" },
        { time: "今日", text: "追加依頼：本文にフォームへのリンクをボタン風に載せたい→[[文字|URL]]でオレンジ背景の押しボタン風リンクに変換できる記法を追加" },
        { time: "今日", text: "追加依頼：お問い合わせボタンを左揃えにしたい→ボタンの揃え位置を中央から左揃えに変更" },
        { time: "今日", text: "追加依頼：オレンジ＋大きめを両方使いたい→***文章***（アスタリスク3つ）で色もサイズも同時に変えられる記法を追加" },
        { time: "今日", text: "追加依頼：太字だけのものも欲しい→****文章****（アスタリスク4つ）で色もサイズも変えず太字だけにできる記法を追加" },
        { time: "今日", text: "質問：送信が途中で止まったらどうなるか→当時は再開の仕組みがなく二重送信の恐れがあると回答。再アプローチリストにA列（送信日）を新設し、送信済みの行は自動スキップ・再実行で続きから送れるように対応完了。人間側の作業として、シートの一番左に列を1本挿入してもらう必要あり" }
      ],
      deliverables: [
  ...
  activity: [
    { time: "今日", who: "ハック", text: "T11：送信が途中で止まっても大丈夫なように、再アプローチリストにA列（送信日）を新設！送信済みの行は自動スキップされ、再実行で続きから送れます🛡️" },
    { time: "今日", who: "ハック", text: "T12：本送信エラーを修正！1セルに複数メールが入っていても自動で拾って個別送信、1件失敗しても止まらないように🛠️" },
    { time: "今日", who: "ハック", text: "T12：担当者名が空欄でも「ご担当者様」で丁寧に送れるようにしました✉️" },
    { time: "今日", who: "ハック", text: "T12：本送信の前に「本当に送っていいですか？」の確認ダイアログを追加！誤送信を防ぎます⚠️" },
    { time: "今日", who: "ハック", text: "T12：Q2にヘッダー画像を貼れるように対応完了！メール上部に画像が表示されます🖼️" },
    { time: "今日", who: "ハック", text: "T12：***の役割を変更！黒字のまま中くらいのサイズ（1.3em）になりました🎨" },
    { time: "今日", who: "ハック", text: "T12：****文章****（アスタリスク4つ）でオレンジ＋大きめを両方使えるように！4段階の装飾が完成しました🎨" },
    { time: "今日", who: "ハック", text: "T12：add oneメルマガと同じボタンリンク機能を移植！[[文字|URL]]でK2本文に直接書けます🔘" },
    { time: "今日", who: "ハック", text: "T12：***文章***（アスタリスク3つ）でオレンジ・サイズそのままの装飾を追加！3段階に🎨" },
    { time: "今日", who: "ハック", text: "T12：強調文字のサイズをT11と同じ1.6emに拡大しました🎨" },
    ...
    { time: "今日", who: "ハック", text: "T11：[[文字|URL]]でフォームリンクをボタン風に表示できるように！オレンジの押しボタンデザインです🔘" },
    { time: "今日", who: "ハック", text: "T11：**は色だけ変更・*はサイズだけ拡大、と役割をきれいに分けました🎨" },
    { time: "今日", who: "ハック", text: "T11：強調文字をもっと大きく調整！画像に近いサイズ感になりました🎨" },
    { time: "今日", who: "ハック", text: "T11：*text*（アスタリスク1つ）で黒字のまま大きさだけ強調できる記法を追加しました🎨" },
    { time: "今日", who: "ハック", text: "T11：本文の一部を強調表示できる**text**記法を追加！オレンジ太字で目立たせられます🎨" },
    { time: "今日", who: "ハック", text: "T11：ご担当者様の様が二重になる不具合に対応。連続した様を自動で1個に畳み込む保険を追加しました✉️" },
